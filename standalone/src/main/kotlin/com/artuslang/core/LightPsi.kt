...
object LightPsi {
    ...
    private class MyParsing internal constructor() : Disposable {
        ...
        fun createFile(name: String, text: String, definition: ParserDefinition): PsiFile? {
            ...
            return (PsiFileFactory.getInstance(project) as PsiFileFactoryImpl).trySetupPsiForFile(LightVirtualFile(name, language, text), language, true, false)
        }
    ...
    object Init {
        ...
        fun initExtensions(project: MockProject) {
            ...
            Extensions.getRootArea().getExtensionPoint<Any>("com.intellij.referencesSearch").registerExtension(CachesBasedRefSearcher())
        ...
        fun initAppAndProject(rootDisposable: Disposable): MockProject {
            ...
            if (component == null) {
                application.picoContainer.registerComponent(object : AbstractComponentAdapter(ProgressManager::class.java.name, Any::class.java) {
            ...
            registerComponentInstance(appContainer, MessageBus::class.java, MessageBusFactory.newMessageBus(application))
            ...
            registerComponentInstance(appContainer, FileTypeManager::class.java, MockFileTypeManager(MockLanguageFileType(PlainTextLanguage.INSTANCE, "txt")))
            ...
            registerApplicationService(project, ReferenceProvidersRegistry::class.java, ReferenceProvidersRegistryImpl::class.java)
            ...
            project.registerService(CachedValuesManager::class.java, CachedValuesManagerImpl(project, PsiCachedValuesFactory(PsiManager.getInstance(project))))
        ...
        fun initApplication(rootDisposable: Disposable): MockApplicationEx {
            ...
            ApplicationManager.setApplication(instance, Getter<FileTypeRegistry> { FileTypeManager.getInstance() }, rootDisposable)
        ...
        fun <T> registerExtensionPoint(area: ExtensionsArea, extensionPointName: ExtensionPointName<T>, aClass: Class<out T>) {
            val name = extensionPointName.name
            if (!area.hasExtensionPoint(name)) {
                val kind = if (aClass.isInterface || aClass.modifiers and Modifier.ABSTRACT != 0)
